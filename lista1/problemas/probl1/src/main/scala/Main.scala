...
@tailrec
def convert_to_rec_aux(num_int: Int, base_to_conv: Int, final_str: String = "") : String = {
  if (num_int == 0) {
    final_str
  }
  else { 
    convert_to_rec_aux(num_int/base_to_conv, base_to_conv, (num_int%base_to_conv) + final_str)
  }
}
...
def convert_to_another_base(num_original: String,
  ...
  val to_base_ten = num_original.reverse.zipWithIndex.map { case (char, index) =>
    values(char) * math.pow(original_base, index).toInt
  }.sum
  
  val num_finally_conv = convert_to_rec_aux(to_base_ten, base_to_conv)
